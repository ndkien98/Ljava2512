...
graph TD
    subgraph VM_Architecture [Kiến trúc Virtual Machine (VM)]
        ...
        subgraph VM1_Box [VM 1]
            GOS1[Guest OS] --> Libs1[Libraries/Bins]
            Libs1 --> App1[App A]
        end

        subgraph VM2_Box [VM 2]
    ...
    subgraph Container_Architecture [Kiến trúc Container (Docker)]
        ...
        subgraph Cont1_Box [Container 1]
            CLibs1[Shared Libraries/Bins] --> CApp1[App A]
        end

        subgraph Cont2_Box [Container 2]
...
1.  **Namespaces (Không gian tên)**: Cô lập tài nguyên hệ thống cho tiến trình.
    - _PID Namespace_: Cô lập danh sách tiến trình (tiến trình trong container không thấy tiến trình bên ngoài).
    - _NET Namespace_: Cô lập card mạng, cổng (port), bảng định tuyến.
    - _MNT Namespace_: Cô lập hệ thống file mount.
    - _IPC Namespace_: Cô lập chia sẻ bộ nhớ giữa các tiến trình.
    - _UTS Namespace_: Cô lập Hostname và Domain name.
    - _USER Namespace_: Cô lập User và Group ID.
...
- `docker ps`: Liệt kê các container đang chạy.
  - _Flag hữu dụng_: `docker ps -a` hiển thị tất cả các container kể cả đã dừng.
...
- `docker logs <container_id_or_name>`: Xem log đầu ra (stdout/stderr) của ứng dụng bên trong container.
  - _Flag hữu dụng_: `docker logs -f --tail 100 <name>` (theo dõi log thời gian thực).
- `docker exec -it <container_id_or_name> <command>`: Thực thi một lệnh trực tiếp bên trong container đang chạy.
  - _Flag hữu dụng_: `-it` (interactive + tty) kết hợp lệnh `/bin/sh` hoặc `/bin/bash` mở terminal tương tác bên trong container.
    - _Ví dụ_: `docker exec -it my-mysql mysql -u root -p`
...
- **RUN**: Thực thi lệnh shell trong quá trình **build** image. Kết quả của lệnh được lưu thành một lớp (layer) mới trong image.
  - _Ví dụ_: `RUN mvn clean package -DskipTests` hoặc `RUN apk add --no-cache curl`
...
- **ENV**: Thiết lập biến môi trường cố định trong suốt quá trình build và chạy container.
  - _Ví dụ_: `ENV SPRING_PROFILES_ACTIVE=prod`
- **CMD vs ENTRYPOINT**:
...
1.  **Docker Volumes**:
    ...
    - _Tạo volume_: `docker volume create db_data`
    - _Gắn vào container_: `docker run -d -v db_data:/var/lib/mysql mysql:8`
2.  **Bind Mounts**:
    ...
    - _Gắn vào container_: `docker run -d -v D:/my-project/html:/usr/share/nginx/html nginx`
3.  **tmpfs Mounts**:
...
- **Host**: Loại bỏ hoàn toàn sự cô lập mạng giữa container và máy host. Container dùng chung cổng và địa chỉ IP trực tiếp của máy host.
  - _Ví dụ_: Chạy Nginx cổng 80 chế độ host thì cổng 80 của máy host sẽ bị chiếm dụng ngay lập tức mà không cần dùng flag `-p`.
...
- _Tính năng quan trọng_: Tất cả các container kết nối vào cùng một Custom Bridge Network có thể tự động phân giải tên miền (DNS Resolution) lẫn nhau bằng **tên Container (Container Name)** thay vì IP.
...
- _Stage 1 (Build)_: Dùng Base Image đầy đủ công cụ để biên dịch source code sang file nhị phân (Jar, Build static files).
- _Stage 2 (Run)_: Dùng Base Image siêu nhẹ (như alpine JRE, nginx-alpine) và chỉ sao chép kết quả đã biên dịch từ Stage 1 sang.
...
#### Mã nguồn docker-compose.yml hoàn chỉnh cho Capstone Project

Dưới đây là tệp `docker-compose.yml` hoàn chỉnh phục vụ dự án Capstone:
...
services:
  mysql-db:
    ...
    volumes:
      - mysql_data:/var/lib/mysql
      - ../source_monolithic/uniqlo-service-v1/src/main/resources/db/database.sql:/docker-entrypoint-initdb.d/init.sql
    networks:
      - uniqlo-network
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "localhost"]
  ...
  config-server:
    ...
    healthcheck:
      test:
        [
          "CMD-SHELL",
          "wget -qO- http://localhost:19088/actuator/health | grep UP || exit 1",
        ]
      interval: 15s
      timeout: 5s
      retries: 5
  ...
  eureka-server:
    ...
    environment:
      ...
      EUREKA_CLIENT_SERVICE_URL_DEFAULTZONE: http://eureka-server:19089/eureka/
    networks:
      - uniqlo-network
    healthcheck:
      test: ["CMD-SHELL", "wget -qO- http://localhost:19089 || exit 1"]
      interval: 15s
      timeout: 5s
      retries: 5
